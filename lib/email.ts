...
const FROM = process.env.SMTP_FROM || `TezTekshir <${process.env.SMTP_USER}>`;
...
export async function sendLoginCode(to: string, code: string) {
  ...
  await transporter.sendMail({
    from: FROM,
    to,
    subject: `${code} — your TezTekshir login code`,
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 24px;">
        <div style="text-align: center; margin-bottom: 24px;">
          <h1 style="margin: 0; font-size: 22px; color: #1e293b;">TezTekshir</h1>
        </div>
        ...
        <p style="color: #94a3b8; font-size: 12px; text-align: center; margin: 0;">
          TezTekshir — AI-powered assessment grading
        </p>
      </div>
    `,
    text: `Your TezTekshir login code is: ${code}\n\nThis code expires in 15 minutes.\n\nIf you didn't request this, you can safely ignore this email.`,
  });
}
...
export async function sendPasswordResetCode(to: string, code: string) {
  ...
  await transporter.sendMail({
    from: FROM,
    to,
    subject: `${code} — reset your TezTekshir password`,
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 24px;">
        <div style="text-align: center; margin-bottom: 24px;">
          <h1 style="margin: 0; font-size: 22px; color: #1e293b;">TezTekshir</h1>
    ...
    `,
    text: `Your TezTekshir password reset code is: ${code}\n\nThis code expires in 15 minutes.\n\nIf you didn't request this, you can safely ignore this email.`,
  });
}
