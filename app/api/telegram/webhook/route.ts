...
export async function POST(req: NextRequest) {
    try {
        ...
        if (text === "/start") {
            await sendMessage(
                chatId,
                `👋 <b>Xush kelibsiz TezTekshir botiga!</b>\n\n` +
                `TezTekshir — sun'iy intellekt yordamida testlarni avtomatik tekshirish tizimi.\n\n` +
        ...
        if (text.startsWith("/create_school")) {
            ...
            if (!user) {
                await sendMessage(
                    chatId,
                    `❌ <b>Hisob topilmadi</b>\n\n` +
                    `Telegram hisobingiz TezTekshir hisobiga ulanmagan.\n\n` +
                    `Avval TezTekshir ilovasida "Sozlamalar" sahifasida Telegram hisobingizni ulang.`
                );
                return NextResponse.json({ ok: true });
            }
            ...
            await sendMessage(
                ...
                `🏫 <b>${school.name}</b>\n\n` +
                `Endi TezTekshir ilovasida direktor paneliga kiring.`
            );
